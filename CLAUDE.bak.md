...
## Purpose
Claude Code Sessions enforces discussion-first AI collaboration through hook-based DAIC methodology with persistent task management, git integration, and specialized agents for complex operations. Available in both Python and JavaScript with complete feature parity.

## Narrative Summary

The cc-sessions package transforms Claude Code into a structured workflow management system with language-agnostic hooks that enforce DAIC (Discussion-Alignment-Implementation-Check) methodology - Claude must discuss approaches and receive explicit approval before implementing code.

**Core Capabilities:**
- **DAIC Enforcement**: Hook-based blocking prevents code edits without explicit triggers ("yert", "make it so", etc.)
- **Todo-Based Boundaries**: Approved TodoWrite lists define exact implementation scope, preventing over-implementation
- **Task Management**: Persistent tasks with git branch enforcement, supporting both single-file and directory-based multi-phase work
- **User Configuration**: Customize trigger phrases, git workflows, blocked tools, and environment settings
- **Templated Protocols**: Auto-adapting protocols based on user configuration (submodules, commit styles, etc.)
- **Specialized Agents**: Context gathering, logging, code review, documentation maintenance
- **Context Preservation**: Automatic compaction and state management across session restarts

Available in both Python and JavaScript with complete feature parity.
