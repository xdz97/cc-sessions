...
function createBackup(projectRoot) {
    ...
    if (fs.existsSync(agentsSrc)) {
        ...
        if (agentCount !== backedUpAgents) {
            console.log(color(`   ✗ Backup verification failed: ${backedUpAgents}/${agentCount} agents backed up`, Colors.RED));
            throw new Error('Backup verification failed - aborting to prevent data loss');
        }
    ...
    if (fs.existsSync(hooksSrc)) {
        ...
        if (hookCount !== backedUpHooks) {
            console.log(color(`   ✗ Backup verification failed: ${backedUpHooks}/${hookCount} hooks backed up`, Colors.RED));
            throw new Error('Backup verification failed - aborting to prevent data loss');
        }
...
`;
    ...
    console.log(color('\n╔════════════════════════════════════════════════════════════════╗', Colors.CYAN));
    console.log(color('║           CC-SESSIONS UNINSTALLER                             ║', Colors.CYAN));
    console.log(color('╚════════════════════════════════════════════════════════════════╝\n', Colors.CYAN));
    ...
    // Confirm
    console.log(color('⚠️  This will remove the cc-sessions framework from this project.', Colors.YELLOW));
    ...
    try {
        ...
        if (typeof backupDir !== 'undefined') {
            console.log(color(`\n📁 Your backup is safe at: ${path.relative(projectRoot, backupDir)}/`, Colors.YELLOW));
        }
        throw e;
    }
}
